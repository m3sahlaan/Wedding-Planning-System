       ...
       {   //checking cart table to see whether item has already been added to cart 
           ...
           if($check_for_duplicates_results=$con->query($check_for_duplicates))         //if there is a record of item added to cart by user previously, 
           {
               if($check_for_duplicates_results->num_rows>0)
               {
                    ...
                    header("location: flower-cha.php"); 
                    exit(0);
               }
               else                     //if there is no record of item in cart 
               {
                    $category = "Flower & Decor";
                    $readFromFlower = "SELECT *
                                                                FROM flower
                                                                WHERE vendor_ID = '$vendorID'
                                                                LIMIT 1";
                    if($readFromFlowerResults=$con->query($readFromFlower))
                    {
                        $info = $readFromFlowerResults->fetch_assoc();
...
<html>
    <head>
        <title>Wedding Flower & Decor</title>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="styles/common.css">
        <link rel="stylesheet" href="styles/flower-cha.css">
    ...
    <body>
        ...
        <section class="header">
                        ...
                        <div class="vendor-cat">
                            ...
                            <div class="dropdown">
                                <ul>                                                    
                                    ...
                                    <li><a class="active" href="flower-cha.php">Flowers & Decor</a></li>
            ...
            <div class="page-name"><a href="flower-cha.php">Wedding Flower & Decor</a></div>
            <div class="search-bar">     
                <form class="search-form" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST">
                    <input class="input" type="text" name="search-text" placeholder="Search Florist Name" size="50px">
