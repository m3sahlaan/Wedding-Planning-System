   ...
   if(isset($_POST['cartbutton']))
   {
       ...
       {   //checking cart table to see whether item has already been added to cart
           ...
           $check_for_duplicates = "SELECT *
                                                     FROM cart
                                                     WHERE user_ID = '$userID' && vendor_ID = '$vendorID' && category = 'DJ'
                                                     LIMIT 1";
           if($check_for_duplicates_results=$con->query($check_for_duplicates))         //if there is a record of item added to cart by user previously,
           {
               if($check_for_duplicates_results->num_rows>0)
               {
                    ...
                    header("location: bands-nim.php");
                    exit(0);
               }
               else                     //if there is no record of item in cart
               {
                    $category = "DJ";
                    $readFromBands = "SELECT *
                                                                FROM bands
                                                                WHERE vendor_ID = '$vendorID'
                                                                LIMIT 1";
                    if($readFromBandsResults=$con->query($readFromBands))
                    {
                        $info = $readFromBandsResults->fetch_assoc();
...
<html>
    <head>
        <title>Wedding DJ &amp; Bands</title>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="styles/common.css">
        <link rel="stylesheet" href="styles/bands-nim.css">
    ...
    <body>
        ...
        <section class="header">
            ...
            <div class="page-name"><a href="bands-nim.php">Wedding DJ &amp; Bands</a></div>
            <div class="search-bar">
                <form class="search-form" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST">
                    <input class="input" type="text" name="search-text" placeholder="Search DJ &amp; Bands" size="50px">
