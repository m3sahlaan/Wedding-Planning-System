   ...
   if(isset($_POST['cartbutton']))
   {
       ...
       {   //checking cart table to see whether item has already been added to cart 
           ...
           $check_for_duplicates = "SELECT *                                    
                                                     FROM cart
                                                     WHERE user_ID = '$userID' && vendor_ID = '$vendorID' && category = 'Venue'
                                                     LIMIT 1";                                
           if($check_for_duplicates_results=$con->query($check_for_duplicates))         //if there is a record of item added to cart by user previously, 
           {
               if($check_for_duplicates_results->num_rows>0)
               {
                    ...
                    header("location: venue-mal.php"); 
                    exit(0);
               }
               else                     //if there is no record of item in cart 
               {
                    $category = "Venue";
                    $readFromVenue = "SELECT *
                                                                FROM venue
                                                                WHERE vendor_ID = '$vendorID'
                                                                LIMIT 1";
                    if($readFromVenueResults=$con->query($readFromVenue))
                    {
                        $info = $readFromVenueResults->fetch_assoc();
...
<html>
    <head>
        ...
        <link rel="stylesheet" href="styles/venue-mal.css">
    ...
    <body>
        ...
        <section class="header">
                ...
                <div class="topnav">
                    <ul>
                        ...
                        <div class="vendor-cat">
                            ...
                            <div class="dropdown">
                                <ul>                                                    
                                    <li><a class="active" href="venue-mal.php">Venues</a></li>
            ...
            <div class="page-name"><a href=" venue-mal.php">Wedding Venues</a></div>
            <div class="search-bar">     
                <form class="search-form" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST">
                    <input class="input" type="text" name="search-text" placeholder="Search Venue" size="50px">
