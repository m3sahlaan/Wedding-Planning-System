   ...
   if(isset($_POST['cartbutton']))
   {
       ...
       {   //checking cart table to see whether item has already been added to cart 
           ...
           $check_for_duplicates = "SELECT *                                    
                                                     FROM cart
                                                     WHERE user_ID = '$userID' && vendor_ID = '$vendorID' && category = '$category' 
                                                     LIMIT 1";                                
           if($check_for_duplicates_results=$con->query($check_for_duplicates))         //if there is a record of item added to cart by user previously, 
           {
               if($check_for_duplicates_results->num_rows>0)
               {
                    ...
                    header("location: officiant-cha.php"); 
                    exit(0);
               }
               else                     //if there is no record of item in cart 
               {
                    $category = "Officiant";
                    $readFromOfficiant = "SELECT *
                                                                FROM officiant
                                                                WHERE vendor_ID = '$vendorID'
                                                                LIMIT 1";
                    if($readFromOfficiantResults=$con->query($readFromOfficiant))
                    {
                        $info = $readFromOfficiantResults->fetch_assoc();
...
<html>
    <head>
        <title>Wedding Officiants</title>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="styles/common.css">
        <link rel="stylesheet" href="styles/officiant-cha.css">
    ...
    <body>
        ...
        <section class="header">
                        ...
                        <div class="vendor-cat">
                            ...
                            <div class="dropdown">
                                <ul>                                                    
                                    ...
                                    <li><a class="active" href="officiant-cha.php">Officiants</a></li>
                                </ul>
                            </div>
                        </div>
            ...
            <div class="page-name"><a href="officiant-cha.php">Wedding Officiants</a></div>
            <div class="search-bar">     
                <form class="search-form" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST">
                    <input class="input" type="text" name="search-text" placeholder="Search Officiant Name" size="50px">
