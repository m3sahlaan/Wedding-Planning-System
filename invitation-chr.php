   ...
   if(isset($_POST['cartbutton']))
   {
       ...
       {   //checking cart table to see whether item has already been added to cart 
           ...
           $check_for_duplicates = "SELECT *                                    
                                                     FROM cart
                                                     WHERE user_ID = '$userID' && vendor_ID = '$vendorID' && category = 'Invitation' 
                                                     LIMIT 1";                                
           if($check_for_duplicates_results=$con->query($check_for_duplicates))         //if there is a record of item added to cart by user previously, 
           {
               if($check_for_duplicates_results->num_rows>0)
               {
                    ...
                    header("location: invitation-chr.php"); 
                    exit(0);
               }
               else                     //if there is no record of item in cart 
               {
                    $category = "Invitation";
                    $readFromInvitation = "SELECT *
                                                                FROM invitation
                                                                WHERE vendor_ID = '$vendorID'
                                                                LIMIT 1";
                    if($readFromInvitationResults=$con->query($readFromInvitation))
                    {
                        $info = $readFromInvitationResults->fetch_assoc();
...
<html>
    <head>
        ...
        <link rel="stylesheet" href="styles/invitation-chr.css">
    ...
    <body>
        ...
        <section class="header">
                        ...
                        <div class="vendor-cat">
                            ...
                            <div class="dropdown">
                                <ul>                                                    
                                    ...
                                    <li><a class="active" href="invitation-chr.php">Invitation</a></li>
            ...
            <div class="page-name"><a href="invitation-chr.php">Wedding Invitation</a></div>
            <div class="search-bar">     
                <form class="search-form" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST">
                    <input class="input" type="text" name="search-text" placeholder="Search Invitation" size="50px">
