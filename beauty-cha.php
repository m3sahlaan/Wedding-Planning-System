       ...
       {   //checking cart table to see whether item has already been added to cart 
           ...
           if($check_for_duplicates_results=$con->query($check_for_duplicates))         //if there is a record of item added to cart by user previously, 
           {
               if($check_for_duplicates_results->num_rows>0)
               {
                    ...
                    header("location: beauty-cha.php"); 
                    exit(0);
               }
               else                     //if there is no record of item in cart 
               {
                    $category = "Beauty & Health";
                    $readFromBeauty = "SELECT *
                                                                FROM beauty
                                                                WHERE vendor_ID = '$vendorID'
                                                                LIMIT 1";
                    if($readFromBeautyResults=$con->query($readFromBeauty))
                    {
                        $info = $readFromBeautyResults->fetch_assoc();
...
<html>
    <head>
        <title>Wedding Beauty & Health</title>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="styles/common.css">
        <link rel="stylesheet" href="styles/beauty-cha.css">
    ...
    <body>
        ...
        <section class="header">
                        ...
                        <div class="vendor-cat">
                            ...
                            <div class="dropdown">
                                <ul>                                                    
                                    ...
                                    <li><a class="active" href="beauty-cha.php">Beauty & Health</a></li>
            ...
            <div class="page-name"><a href="beauty-cha.php">Wedding Beauty & Health</a></div>
            <div class="search-bar">     
                <form class="search-form" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST">
                    <input class="input" type="text" name="search-text" placeholder="Search Beautician Name" size="50px">
