   ...
   if(isset($_POST['cartbutton']))
   {
       ...
       {   //checking cart table to see whether item has already been added to cart 
           ...
           $check_for_duplicates = "SELECT *                                    
                                                     FROM cart
                                                     WHERE user_ID = '$userID' && vendor_ID = '$vendorID' && category = 'Bridal Wear'
                                                     LIMIT 1";                                
           if($check_for_duplicates_results=$con->query($check_for_duplicates))         //if there is a record of item added to cart by user previously, 
           {
               if($check_for_duplicates_results->num_rows>0)
               {
                    ...
                    header("location: bridal-wear-mal.php"); 
                    exit(0);
               }
               else                     //if there is no record of item in cart 
               {
                    $category = "Bridal Wear";
                    $readFromBridalWear = "SELECT *
                                                                FROM bridal_wear
                                                                WHERE vendor_ID = '$vendorID'
                                                                LIMIT 1";
                    if($readFromBridalWearResults=$con->query($readFromBridalWear))
                    {
                        $info = $readFromBridalWearResults->fetch_assoc();
...
<html>
    <head>
        ...
        <link rel="stylesheet" href="styles/bridal-wear-mal.css">
                        ...
                        <div class="vendor-cat">
                            ...
                            <div class="dropdown">
                                <ul>                                                    
                                    ...
                                    <li><a class="active" href="bridal-wear-mal.php">Bridal Wear</a></li>
